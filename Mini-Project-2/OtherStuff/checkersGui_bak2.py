...
def _section_label(parent, text):
    tk.Label(parent, text=text, font=('Segoe UI', 8, 'bold'),
             bg=THEME['bg'], fg=THEME['text_muted']).pack(anchor='w', pady=(16, 4))


def _card(parent):
    f = tk.Frame(parent, bg=THEME['card'], bd=0)
    f.pack(fill=tk.X)
    return f
...
class ConfigScreen:
    ...
    def _build(self):
        ...
        outer.pack(padx=36, pady=36, fill=tk.BOTH)

        tk.Label(outer, text='CHECKERS', font=FONT_TITLE,
                 bg=THEME['bg'], fg=THEME['gold']).pack()
        tk.Label(outer, text='Configure your game below',
                 font=FONT_SMALL, bg=THEME['bg'], fg=THEME['text_muted']).pack(pady=(2, 0))

        _section_label(outer, 'GAME MODE')
        mode_card = _card(outer)
        for label, val in [('Human vs Human', 'human'),
                           ('Human vs AI  (you play White)', 'ai')]:
            tk.Radiobutton(mode_card, text=label, variable=self._mode, value=val,
                           command=self._toggle_ai,
                           font=FONT_BODY, bg=THEME['card'], fg=THEME['text'],
                           selectcolor=THEME['bg'],
                           activebackground=THEME['card'],
                           activeforeground=THEME['accent']).pack(anchor='w', padx=16, pady=8)

        _section_label(outer, 'AI CONFIGURATION')
        self._ai_card = tk.Frame(outer, bg=THEME['card'], bd=0)

        tk.Label(self._ai_card, text='Search Strategy',
                 font=('Segoe UI', 9, 'bold'), bg=THEME['card'],
                 fg=THEME['text_dim']).pack(anchor='w', padx=16, pady=(12, 4))
        for label, val in [('Minimax', 'Minimax'),
                           ('Alpha-Beta Pruning', 'AlphaBeta'),
                           ('Alpha-Beta + Move Ordering  (recommended)', 'AlphaBetaOrdering')]:
            tk.Radiobutton(self._ai_card, text=label, variable=self._strategy, value=val,
                           font=FONT_SMALL, bg=THEME['card'], fg=THEME['text'],
                           selectcolor=THEME['bg'],
                           activebackground=THEME['card'],
                           activeforeground=THEME['accent']).pack(anchor='w', padx=28, pady=2)

        tk.Frame(self._ai_card, bg=THEME['border'], height=1).pack(fill=tk.X, padx=16, pady=10)

        depth_row = tk.Frame(self._ai_card, bg=THEME['card'])
        depth_row.pack(fill=tk.X, padx=16, pady=(0, 4))
        tk.Label(depth_row, text='Search Depth', font=('Segoe UI', 9, 'bold'),
                 bg=THEME['card'], fg=THEME['text_dim']).pack(side=tk.LEFT)
        tk.Label(depth_row, textvariable=self._depth, width=3,
                 font=('Segoe UI', 10, 'bold'),
                 bg=THEME['card'], fg=THEME['accent']).pack(side=tk.RIGHT)
        tk.Scale(self._ai_card, variable=self._depth, from_=5, to=9,
                 orient=tk.HORIZONTAL, showvalue=False,
                 bg=THEME['card'], fg=THEME['text'],
                 troughcolor=THEME['border'], activebackground=THEME['accent'],
                 highlightthickness=0, bd=0).pack(fill=tk.X, padx=16, pady=(0, 8))

        time_row = tk.Frame(self._ai_card, bg=THEME['card'])
        time_row.pack(fill=tk.X, padx=16, pady=(0, 4))
        tk.Label(time_row, text='Time Limit (seconds)', font=('Segoe UI', 9, 'bold'),
                 bg=THEME['card'], fg=THEME['text_dim']).pack(side=tk.LEFT)
        tk.Label(time_row, textvariable=self._time, width=4,
                 font=('Segoe UI', 10, 'bold'),
                 bg=THEME['card'], fg=THEME['accent']).pack(side=tk.RIGHT)
        tk.Scale(self._ai_card, variable=self._time, from_=1.0, to=3.0,
                 resolution=0.5, orient=tk.HORIZONTAL, showvalue=False,
                 bg=THEME['card'], fg=THEME['text'],
                 troughcolor=THEME['border'], activebackground=THEME['accent'],
                 highlightthickness=0, bd=0).pack(fill=tk.X, padx=16, pady=(0, 14))

        tk.Button(outer, text='START  GAME',
                  font=FONT_HEADER, fg='#FFFFFF',
                  bg=THEME['accent'], activebackground=THEME['accent_hover'],
                  activeforeground='#FFFFFF',
                  relief=tk.FLAT, bd=0, padx=22, pady=12,
                  cursor='hand2', command=self._start).pack(fill=tk.X, pady=(20, 0))

        self._toggle_ai()

    def _toggle_ai(self):
        if self._mode.get() == 'ai':
            self._ai_card.pack(fill=tk.X)
        else:
...
class CheckerGUI:
    ...
    def _build_analytics_panel(self, parent):
        ...
        def _draw(_event=None):
            ...
            for label, val in [('Strategy', self.ai_strategy_name),
                ...
                y += 20
            outer.create_line(16, y + 4, w - 16, y + 4, fill=THEME['border'], width=1)
            y += 16
    ...
    def _build_history_panel(self, parent):
        wrapper = tk.Frame(parent, bg=THEME['card'], bd=0)
    ...
    def _build_buttons(self, parent):
        ...
        tk.Button(btn_frame, text='NEW  GAME',
                  ...
                  cursor='hand2', command=self.new_game).pack(side=tk.LEFT, fill=tk.X,
                                                              expand=True, padx=(0, 6))
        tk.Button(btn_frame, text='MENU',
                  font=FONT_HEADER, fg=THEME['text'],
                  bg=THEME['card'], activebackground=THEME['card_alt'],
                  activeforeground=THEME['text'],
                  relief=tk.FLAT, bd=0, padx=16, pady=10,
                  cursor='hand2', command=self.go_to_menu).pack(side=tk.LEFT)
        ...
        if self.selected_pos:
            ...
            for tr, tc in self.valid_moves:
                ...
                if abs(tr - fr) == 2:
                    ...
                    c.create_text(xc, yc, text='x', font=('Segoe UI', 16, 'bold'),
                                  fill='#FFFFFF')
                else:
            ...
            for col in range(BOARD_SIZE):
                ...
                if piece == WHITE_KING:
                    c.create_text(xc, yc + 1, text='K',
                                  font=('Segoe UI', 13, 'bold'), fill=THEME['gold'])
                elif piece == BLACK_KING:
                    c.create_text(xc, yc + 1, text='K',
                                  font=('Segoe UI', 13, 'bold'), fill=THEME['piece_ring_b'])
    ...
    def add_move_to_history(self, player, start, end, was_jump, promoted, is_ai=False):
        ...
        icon   = 'o' if player == WHITE else '.'
        ai_tag = ' AI' if is_ai else ''
        h.insert(tk.END, f' {self.move_counter:>3}. {icon}{ai_tag} ', tag)
        h.insert(tk.END, f'({start[0]+1},{start[1]+1})->({end[0]+1},{end[1]+1})')
        if was_jump:
    ...
    def update_analytics_display(self, analytics):
        ...
        ac.itemconfig(self.analytics_labels['nodes'],  text=f'{analytics.NumberNodesExpanded:,}')
        ac.itemconfig(self.analytics_labels['prunes'], text=f'{analytics.NumberNodesPruned:,}')
        ac.itemconfig(self.analytics_labels['depth'],  text=f'{analytics.MaxDepthReached}')
        elapsed = analytics.GetTimeElapsed() if hasattr(analytics, 'GetTimeElapsed') else 0
        ac.itemconfig(self.analytics_labels['time'],   text=f'{elapsed:.3f}s')

    def check_game_over(self):
        white_n = sum(1 for row in self.game_board.board for c in row if c.upper() == WHITE)
        black_n = sum(1 for row in self.game_board.board for c in row if c.upper() == BLACK)
        winner = None
        if white_n == 0:
            winner = 'BLACK'
        elif black_n == 0:
    ...
    def _save_log(self, winner=None):
        ...
        history = [ln.strip() for ln in self.history_text.get('1.0', tk.END).strip().splitlines()
                   if ln.strip()]
        path = save_game_log({
...
def launch():
    while True:
        ...
        if config_screen.result is None:
            break
        game = CheckerGUI(**config_screen.result)
